...
class GlobalOffloadSession {
  public:
    ...
    std::shared_ptr<OffloadBundleContext> createSession(const OffloadBundleEffectType& type, int statusDepth,
        ...
        auto context = std::make_shared<OffloadBundleContext>(statusDepth, common, type);
  ...
  private:
    ...
    std::unordered_map<int /* session ID */, std::vector<std::shared_ptr<OffloadBundleContext>>>

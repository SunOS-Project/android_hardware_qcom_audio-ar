...
class XmlConverter {
   public:
    XmlConverter(const std::string& configFilePath,
        ...
        : XmlConverter(configFilePath,
                       ::android::audio_is_readable_configuration_file(
   ...
   private:
    XmlConverter(
        const std::string& configFilePath, const bool& isReadableConfigFile,
        const std::function<std::optional<T>(const char*)>& readXmlConfig)
        : mXsdcConfig{isReadableConfigFile
                          ? readXmlConfig(configFilePath.c_str())
                          : std::nullopt},
          mStatus(mXsdcConfig ? ::android::OK : ::android::NO_INIT),
          mErrorMessage(
              generateError(configFilePath, isReadableConfigFile, mStatus)) {}
    ...
    static std::string generateError(const std::string& configFilePath,
        ...
        if (status != ::android::OK) {
            if (!isReadableConfigFile) {
                errorMessage =
                    "Could not read requested config file:" + configFilePath;
            } else {
...
static std::vector<A> convertWrappedCollectionToAidl(
    ...
    if (!xsdcWrapperTypeVec.empty()) {
         ...
         */
        resultAidlTypeVec.reserve(
            getInnerTypeVec(xsdcWrapperTypeVec[0]).size());
        for (const W& xsdcWrapperType : xsdcWrapperTypeVec) {
            std::transform(getInnerTypeVec(xsdcWrapperType).begin(),
                           ...
                           std::back_inserter(resultAidlTypeVec),
                           convertToAidl);
        }
    }
...
static std::vector<A> convertCollectionToAidl(
    const std::vector<X>& xsdcTypeVec,
    std::function<A(const X&)> convertToAidl) {
    std::vector<A> resultAidlTypeVec;
    resultAidlTypeVec.reserve(xsdcTypeVec.size());
    std::transform(xsdcTypeVec.begin(), xsdcTypeVec.end(),

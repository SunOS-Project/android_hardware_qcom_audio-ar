...
class StreamContext {
   public:
    ...
    ::aidl::android::media::audio::common::AudioChannelLayout getChannelLayout()
        const {
    ...
    bool getForceTransientBurst() const {
        return mDebugParameters.forceTransientBurst;
    }
    bool getForceSynchronousDrain() const {
        return mDebugParameters.forceSynchronousDrain;
    }
    ...
    int getTransientStateDelayMs() const {
        return mDebugParameters.transientStateDelayMs;
    }
...
struct DriverInterface {
    using CreateInstance =
        std::function<DriverInterface*(const StreamContext&)>;
    virtual ~DriverInterface() = default;
    // This function is called once, on the main thread, before starting the
    // worker thread.
    virtual ::android::status_t init() = 0;
    // This function is called from Binder pool thread. It must be done in a
    // thread-safe manner if this method and other methods in this interface
    // share data.
...
class StreamWorkerCommonLogic
    : public ::android::hardware::audio::common::StreamLogic {
   public:
    bool isClosed() const {
        return static_cast<int32_t>(mState.load()) ==
               StreamContext::STATE_CLOSED;
    }
    void setClosed() {
        mState = static_cast<
            ::aidl::android::hardware::audio::core::StreamDescriptor::State>(
            StreamContext::STATE_CLOSED);
    }
   ...
   protected:
    ...
    StreamWorkerCommonLogic(const StreamContext& context,
    ...
    void populateReplyWrongState(
        ::aidl::android::hardware::audio::core::StreamDescriptor::Reply* reply,
        const ::aidl::android::hardware::audio::core::StreamDescriptor::Command&
            command) const;
    ...
    static_assert(
        std::atomic<::aidl::android::hardware::audio::core::StreamDescriptor::
                        State>::is_always_lock_free);
    std::atomic<::aidl::android::hardware::audio::core::StreamDescriptor::State>
        mState = ::aidl::android::hardware::audio::core::StreamDescriptor::
            State::STANDBY;
...
struct StreamWorkerInterface {
    using CreateInstance = std::function<StreamWorkerInterface*(
        const StreamContext& context, DriverInterface* driver)>;
...
class StreamWorkerImpl
    : public StreamWorkerInterface,
      public ::android::hardware::audio::common::StreamWorker<WorkerLogic> {
    using WorkerImpl =
        ::android::hardware::audio::common::StreamWorker<WorkerLogic>;
   ...
   public:
    ...
    void setIsConnected(bool isConnected) override {
        WorkerImpl::setIsConnected(isConnected);
    }
    void setClosed() override { WorkerImpl::setClosed(); }
    bool start() override {
        return WorkerImpl::start(WorkerImpl::kThreadName,
...
// This provides a C++ interface with methods of the IStreamCommon Binder
// interface, but intentionally does not inherit from it. This is needed to
// avoid inheriting StreamIn and StreamOut from two Binder interface classes, as
// these parts of the class will be reference counted separately.
//
// The implementation of these common methods is in the StreamCommonImpl
// template class.
...
class StreamCommon
    : public ::aidl::android::hardware::audio::core::BnStreamCommon {
   public:
    explicit StreamCommon(
   ...
   private:
    ...
    ndk::ScopedAStatus getVendorParameters(
        ...
        auto delegate = mDelegate.lock();
        return delegate != nullptr
                   ? delegate->getVendorParameters(in_ids, _aidl_return)
                   : ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_STATE);
    }
    ndk::ScopedAStatus setVendorParameters(
        ...
        auto delegate = mDelegate.lock();
        return delegate != nullptr
                   ? delegate->setVendorParameters(in_parameters, in_async)
                   : ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_STATE);
    }
    ...
    ndk::ScopedAStatus removeEffect(
        ...
        auto delegate = mDelegate.lock();
        return delegate != nullptr
                   ? delegate->removeEffect(in_effect)
                   : ndk::ScopedAStatus::fromExceptionCode(EX_ILLEGAL_STATE);
    }
...
class StreamIn : public StreamCommonImpl<
    ...
    ndk::ScopedAStatus getActiveMicrophones(
        std::vector<
            ::aidl::android::media::audio::common::MicrophoneDynamicInfo>*
            _aidl_return) override;
    ndk::ScopedAStatus getMicrophoneDirection(
        MicrophoneDirection* _aidl_return) override;
    ndk::ScopedAStatus setMicrophoneDirection(
        MicrophoneDirection in_direction) override;
    ndk::ScopedAStatus getMicrophoneFieldDimension(
    ...
    ndk::ScopedAStatus setHwGain(
        const std::vector<float>& in_channelGains) override;

   protected:
    friend class ndk::SharedRefBase;

    static ndk::ScopedAStatus initInstance(
        const std::shared_ptr<StreamIn>& stream);

    StreamIn(const ::aidl::android::hardware::audio::common::SinkMetadata&
                 sinkMetadata,
    ...
    void createStreamCommon(const std::shared_ptr<StreamIn>& myPtr) {
        StreamCommonImpl<::aidl::android::hardware::audio::common::
                             SinkMetadata>::createStreamCommon(myPtr);
    }

    const std::map<::aidl::android::media::audio::common::AudioDevice,
                   std::string>
        mMicrophones;
...
class StreamOut : public StreamCommonImpl<
    ...
    ndk::ScopedAStatus getAudioDescriptionMixLevel(
        float* _aidl_return) override;
    ...
    ndk::ScopedAStatus getRecommendedLatencyModes(
        std::vector<::aidl::android::media::audio::common::AudioLatencyMode>*
            _aidl_return) override;
    ...
    ndk::ScopedAStatus selectPresentation(int32_t in_presentationId,
                                          int32_t in_programId) override;

    void createStreamCommon(const std::shared_ptr<StreamOut>& myPtr) {
        StreamCommonImpl<::aidl::android::hardware::audio::common::
                             SourceMetadata>::createStreamCommon(myPtr);
    }
   ...
   protected:
    ...
    static ndk::ScopedAStatus initInstance(
        const std::shared_ptr<StreamOut>& stream);

    StreamOut(const ::aidl::android::hardware::audio::common::SourceMetadata&
                  sourceMetadata,
              StreamContext&& context,
              const DriverInterface::CreateInstance& createDriver,
              const StreamWorkerInterface::CreateInstance& createWorker,
    ...
    std::optional<::aidl::android::media::audio::common::AudioOffloadInfo>
        mOffloadInfo;
    std::optional<
        ::aidl::android::hardware::audio::common::AudioOffloadMetadata>
        mOffloadMetadata;
   ...
   public:
    using CreateInstance = std::function<ndk::ScopedAStatus(
        const ::aidl::android::hardware::audio::common::SourceMetadata&
            sourceMetadata,
        StreamContext&& context,
        const std::optional<::aidl::android::media::audio::common::
...
class Streams {
   public:
    ...
    size_t count(int32_t id) {
        ...
        erase_if(mStreams,
                 [](const auto& pair) { return !pair.second.isStreamOpen(); });

...
void StreamContext::fillDescriptor(StreamDescriptor* desc) {
    ...
    if (mDataMQ) {
        ...
        desc->bufferSizeFrames =
            mDataMQ->getQuantumCount() * mDataMQ->getQuantumSize() / frameSize;
        desc->audio.set<StreamDescriptor::AudioBuffer::Tag::fmq>(
            mDataMQ->dupeDesc());
    }
}
...
std::string StreamWorkerCommonLogic::init() {
    ...
    if (sizeof(DataBufferElement) != mDataMQ->getQuantumSize()) {
        return "Unexpected Data MQ quantum size: " +
...
StreamInWorkerLogic::Status StreamInWorkerLogic::cycle() {
    ...
    const LogSeverity severity =
        command.getTag() == Tag::burst || command.getTag() == Tag::getStatus
            ? LogSeverity::VERBOSE
            : LogSeverity::DEBUG;
    LOG(severity) << __func__ << ": received command " << command.toString()
                  << " in " << kThreadName;
    StreamDescriptor::Reply reply{};
    ...
    switch (command.getTag()) {
        case Tag::halReservedExit:
            if (const int32_t cookie = command.get<Tag::halReservedExit>();
                ...
                LOG(WARNING) << __func__
                             << ": EXIT command has a bad cookie: " << cookie;
            }
            break;
        case Tag::getStatus:
        ...
        case Tag::burst:
            if (const int32_t fmqByteCount = command.get<Tag::burst>();
                ...
                LOG(VERBOSE) << __func__ << ": '" << toString(command.getTag())
                             << "' command for " << fmqByteCount << " bytes";
                ...
                LOG(WARNING) << __func__
                             << ": invalid burst byte count: " << fmqByteCount;
            }
            break;
        case Tag::drain:
            if (const auto mode = command.get<Tag::drain>();
                ...
                LOG(WARNING)
                    << __func__ << ": invalid drain mode: " << toString(mode);
            }
            break;
        case Tag::standby:
            if (mState == StreamDescriptor::State::IDLE) {
        ...
        case Tag::pause:
            if (mState == StreamDescriptor::State::ACTIVE) {
                if (::android::status_t status = mDriver->pause();
                    status == ::android::OK) {
                    populateReply(&reply, mIsConnected);
                    mState = StreamDescriptor::State::PAUSED;
                } else {
        ...
        case Tag::flush:
            if (mState == StreamDescriptor::State::PAUSED) {
                if (::android::status_t status = mDriver->flush();
                    status == ::android::OK) {
                    populateReply(&reply, mIsConnected);
                    mState = StreamDescriptor::State::STANDBY;
                } else {
    ...
    if (!mReplyMQ->writeBlocking(&reply, 1)) {
        LOG(ERROR) << __func__ << ": writing of reply " << reply.toString()
                   << " to MQ failed";
        mState = StreamDescriptor::State::ERROR;
        return Status::ABORT;
    }
...
bool StreamInWorkerLogic::read(size_t clientSize,
                               StreamDescriptor::Reply* reply) {
    const size_t byteCount =
        std::min({clientSize, mDataMQ->availableToWrite(), mDataBufferSize});
    ...
    if (isConnected) {
        if (::android::status_t status =
                mDriver->transfer(mDataBuffer.get(), byteCount / mFrameSize,
    ...
    if (bool success = actualByteCount > 0
                           ? mDataMQ->write(&mDataBuffer[0], actualByteCount)
                           : true;
        success) {
        LOG(VERBOSE) << __func__ << ": writing of " << actualByteCount
                     << " bytes into data MQ"
    ...
    if (mState == StreamDescriptor::State::DRAINING ||
        ...
        if (auto stateDurationMs =
            ...
            if (mAsyncCallback == nullptr) {
                ...
                if (mState == StreamDescriptor::State::DRAINING) {
                    ...
                    ndk::ScopedAStatus status =
                        mAsyncCallback->onTransferReady();
                    if (!status.isOk()) {
                        LOG(ERROR)
                            << __func__
            ...
            if (mTransientStateDelayMs.count() != 0) {
                LOG(DEBUG) << __func__ << ": switched to state "
                           << toString(mState) << " after a timeout";
            }
        }
    }
...
bool StreamOutWorkerLogic::write(size_t clientSize,
                                 StreamDescriptor::Reply* reply) {
    const size_t readByteCount = mDataMQ->availableToRead();
    bool fatal = false;
    int32_t latency = Module::kLatencyMs;
    if (bool success = readByteCount > 0
                           ? mDataMQ->read(&mDataBuffer[0], readByteCount)
                           : true) {
        const bool isConnected = mIsConnected;
        LOG(VERBOSE) << __func__ << ": reading of " << readByteCount
                     << " bytes from data MQ"
                     << " succeeded; connected? " << isConnected;
        // Amount of data that the HAL module is going to actually use.
        size_t byteCount =
            std::min({clientSize, readByteCount, mDataBufferSize});
...
StreamCommonImpl<Metadata>::~StreamCommonImpl() {
    if (!isClosed()) {
        LOG(ERROR)
            << __func__
            << ": stream was not closed prior to destruction, resource leak";
        stopWorker();
...
void StreamCommonImpl<Metadata>::createStreamCommon(
    ...
    if (mCommon != nullptr) {
        LOG(FATAL) << __func__
                   << ": attempting to create the common interface twice";
    }
    mCommon = ndk::SharedRefBase::make<StreamCommon>(delegate);
    mCommonBinder = mCommon->asBinder();
    AIBinder_setMinSchedulerPolicy(mCommonBinder.get(), SCHED_NORMAL,
...
void StreamCommonImpl<Metadata>::stopWorker() {
    if (auto commandMQ = mContext.getCommandMQ(); commandMQ != nullptr) {
        ...
        if (!commandMQ->writeBlocking(&cmd, 1)) {
            LOG(ERROR) << __func__
                       << ": failed to write exit command to the MQ";
        }
...
template <class Metadata>
ndk::ScopedAStatus StreamCommonImpl<Metadata>::updateMetadata(
...
// static
ndk::ScopedAStatus StreamIn::initInstance(
...
static std::map<AudioDevice, std::string> transformMicrophones(
    ...
    std::transform(microphones.begin(), microphones.end(),
                   std::inserter(result, result.begin()), [](const auto& mic) {
                       return std::make_pair(mic.device, mic.id);
                   });
    return result;
}
}  // namespace
...
StreamIn::StreamIn(const SinkMetadata& sinkMetadata, StreamContext&& context,
                   ...
                   const std::vector<MicrophoneInfo>& microphones)
    : StreamCommonImpl<SinkMetadata>(sinkMetadata, std::move(context),
                                     createDriver, createWorker),
...
ndk::ScopedAStatus StreamIn::getMicrophoneDirection(
...
ndk::ScopedAStatus StreamIn::setMicrophoneDirection(
...
ndk::ScopedAStatus StreamIn::setHwGain(
    const std::vector<float>& in_channelGains) {
    LOG(DEBUG) << __func__ << ": gains "
               << ::android::internal::ToString(in_channelGains);
    return ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
}

// static
ndk::ScopedAStatus StreamOut::initInstance(
...
StreamOut::StreamOut(const SourceMetadata& sourceMetadata,
    ...
    : StreamCommonImpl<SourceMetadata>(sourceMetadata, std::move(context),
                                       createDriver, createWorker),
...
ndk::ScopedAStatus StreamOut::setHwVolume(
...
ndk::ScopedAStatus StreamOut::getPlaybackRateParameters(
    AudioPlaybackRate* _aidl_return) {
    LOG(DEBUG) << __func__;
    (void)_aidl_return;
    return ndk::ScopedAStatus::fromExceptionCode(EX_UNSUPPORTED_OPERATION);
}
...
ndk::ScopedAStatus StreamOut::selectPresentation(int32_t in_presentationId,
                                                 int32_t in_programId) {
    LOG(DEBUG) << __func__ << ": presentationId " << in_presentationId

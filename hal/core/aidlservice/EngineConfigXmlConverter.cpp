...
#define STRATEGY_ENTRY(name) \
    {"STRATEGY_" #name, static_cast<int>(AudioProductStrategyType::name)}
...
bool isDefaultAudioAttributes(const AudioAttributes& attributes) {
    return ((attributes.contentType == AudioContentType::UNKNOWN) &&
            ...
            (attributes.source == AudioSource::DEFAULT) &&
            (attributes.flags == 0) && (attributes.tags.empty()));
}
...
AudioAttributes EngineConfigXmlConverter::convertAudioAttributesToAidl(
    ...
    if (xsdcAudioAttributes.hasAttributesRef()) {
        if (mAttributesReferenceMap.empty()) {
            mAttributesReferenceMap =
                generateReferenceMap<xsd::AttributesRef,
                                     xsd::AttributesRefType>(
    ...
    if (xsdcAudioAttributes.hasUsage()) {
        aidlAudioAttributes.usage = static_cast<AudioUsage>(
            xsdcAudioAttributes.getFirstUsage()->getValue());
    }
    if (xsdcAudioAttributes.hasSource()) {
        aidlAudioAttributes.source = static_cast<AudioSource>(
    ...
    if (xsdcAudioAttributes.hasFlags()) {
        ...
        for (const xsd::FlagType& xsdcFlagType : xsdcFlagTypeVec) {
            if (xsdcFlagType != xsd::FlagType::AUDIO_FLAG_NONE) {
                aidlAudioAttributes.flags |=
                    1 << (static_cast<int>(xsdcFlagType) - 1);
            }
        }
    }
    if (xsdcAudioAttributes.hasBundle()) {
        const xsd::BundleType* xsdcBundle =
            xsdcAudioAttributes.getFirstBundle();
        aidlAudioAttributes.tags[0] =
            xsdcBundle->getKey() + "=" + xsdcBundle->getValue();
    }
...
AudioHalAttributesGroup EngineConfigXmlConverter::convertAttributesGroupToAidl(
    ...
    aidlAttributesGroup.streamType = static_cast<AudioStreamType>(
        static_cast<int>(xsdcAttributesGroup.getStreamType()) -
        kStreamTypeEnumOffset);
    ...
    if (xsdcAttributesGroup.hasAttributes_optional()) {
        aidlAttributesGroup.attributes =
            convertCollectionToAidl<xsd::AttributesType, AudioAttributes>(
                ...
                std::bind(
                    &EngineConfigXmlConverter::convertAudioAttributesToAidl,
        ...
        aidlAttributesGroup.attributes.push_back(
            convertAudioAttributesToAidl(xsd::AttributesType(
        ...
        // Currently, attributes are not mandatory in schema, but an
        // AttributesGroup without attributes does not make much sense.
    }
...
AudioHalProductStrategy EngineConfigXmlConverter::convertProductStrategyToAidl(
    ...
    if (xsdcProductStrategy.hasAttributesGroup()) {
        aidlProductStrategy.attributesGroups = convertCollectionToAidl<
            xsd::AttributesGroup, AudioHalAttributesGroup>(
            xsdcProductStrategy.getAttributesGroup(),
            std::bind(&EngineConfigXmlConverter::convertAttributesGroupToAidl,
                      this, std::placeholders::_1));
    }
    if ((mDefaultProductStrategyId != std::nullopt) &&
        (mDefaultProductStrategyId.value() == -1)) {
        mDefaultProductStrategyId = aidlProductStrategy.id;
    }
    return aidlProductStrategy;
}

AudioHalVolumeCurve::CurvePoint
EngineConfigXmlConverter::convertCurvePointToAidl(
...
AudioHalVolumeCurve EngineConfigXmlConverter::convertVolumeCurveToAidl(
    ...
    aidlVolumeCurve.deviceCategory =
        static_cast<AudioHalVolumeCurve::DeviceCategory>(
            xsdcVolumeCurve.getDeviceCategory());
    if (xsdcVolumeCurve.hasRef()) {
        if (mVolumesReferenceMap.empty()) {
            mVolumesReferenceMap =
                generateReferenceMap<xsd::VolumesType, xsd::VolumeRef>(
                    getXsdcConfig()->getVolumes());
        }
        aidlVolumeCurve.curvePoints =
            convertCollectionToAidl<std::string,
                                    AudioHalVolumeCurve::CurvePoint>(
                mVolumesReferenceMap.at(xsdcVolumeCurve.getRef()).getPoint(),
                std::bind(&EngineConfigXmlConverter::convertCurvePointToAidl,
                          this, std::placeholders::_1));
    } else {
        aidlVolumeCurve.curvePoints =
            convertCollectionToAidl<std::string,
...
AudioHalVolumeGroup EngineConfigXmlConverter::convertVolumeGroupToAidl(
    ...
    aidlVolumeGroup.volumeCurves =
        convertCollectionToAidl<xsd::Volume, AudioHalVolumeCurve>(
...
EngineConfigXmlConverter::convertCapCriterionTypeToAidl(
    ...
    aidlCapCriterionType.isInclusive =
        !(static_cast<bool>(xsdcCriterionType.getType()));
    aidlCapCriterionType.values = convertWrappedCollectionToAidl<
        xsd::ValuesType, xsd::ValueType, std::string>(
        xsdcCriterionType.getValues(), &xsd::ValuesType::getValue,
        std::bind(&EngineConfigXmlConverter::convertCriterionTypeValueToAidl,
                  this, std::placeholders::_1));
...
void EngineConfigXmlConverter::init() {
    ...
    if (getXsdcConfig()->hasProductStrategies()) {
        mAidlEngineConfig.productStrategies = convertWrappedCollectionToAidl<
            xsd::ProductStrategies, xsd::ProductStrategies::ProductStrategy,
            AudioHalProductStrategy>(
            getXsdcConfig()->getProductStrategies(),
            &xsd::ProductStrategies::getProductStrategy,
            std::bind(&EngineConfigXmlConverter::convertProductStrategyToAidl,
                      this, std::placeholders::_1));
        if (mDefaultProductStrategyId) {
            mAidlEngineConfig.defaultProductStrategyId =
                mDefaultProductStrategyId.value();
        }
    }
    if (getXsdcConfig()->hasVolumeGroups()) {
        mAidlEngineConfig.volumeGroups =
            convertWrappedCollectionToAidl<xsd::VolumeGroupsType,
                                           xsd::VolumeGroupsType::VolumeGroup,
                                           AudioHalVolumeGroup>(
                getXsdcConfig()->getVolumeGroups(),
                &xsd::VolumeGroupsType::getVolumeGroup,
                std::bind(&EngineConfigXmlConverter::convertVolumeGroupToAidl,
                          this, std::placeholders::_1));
    }
    if (getXsdcConfig()->hasCriteria() &&
        getXsdcConfig()->hasCriterion_types()) {
        AudioHalEngineConfig::CapSpecificConfig capSpecificConfig;
        capSpecificConfig.criteria = convertWrappedCollectionToAidl<
            xsd::CriteriaType, xsd::CriterionType, AudioHalCapCriterion>(
            getXsdcConfig()->getCriteria(), &xsd::CriteriaType::getCriterion,
            std::bind(&EngineConfigXmlConverter::convertCapCriterionToAidl,
                      this, std::placeholders::_1));
        capSpecificConfig.criterionTypes =
            convertWrappedCollectionToAidl<xsd::CriterionTypesType,
                                           xsd::CriterionTypeType,
                                           AudioHalCapCriterionType>(

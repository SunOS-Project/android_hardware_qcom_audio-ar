...
static AudioProfile createProfile(const std::string& name, PcmType pcmType,
                        const std::vector<int32_t>& channelLayouts,
                        const std::vector<int64_t>& sampleRates,
...
static AudioProfile createProfile(const std::string& name, const std::string& encodingType,
                        const std::vector<int32_t>& channelLayouts,
                        const std::vector<int64_t>& sampleRates,
                        AudioEncapsulationType encapsulationType = AudioEncapsulationType::NONE) {
    AudioProfile profile;
...
static void dumpProfiles(const AudioProfile& profile, int32_t portId) {
    ...
    std::for_each(profile.sampleRates.begin(), profile.sampleRates.end(),
        [&](int32_t rate) {
            sampleRates += std::to_string(rate) + ", ";
        });
...
static void dumpRoute(const AudioRoute& route) {
    ...
    std::for_each(route.sourcePortIds.begin(), route.sourcePortIds.end(),
        [&](int32_t rate) {
            sourcePorts += std::to_string(rate) + ", ";
        });
...
static void dumpDeviceExt(const AudioPortExt& ext) {
    ...
    LOG(DEBUG) << "DeviceExt: flags: 0x" << std::hex << deviceExt.flags << " address: "
...
static void dumpPort(const AudioPort& port, bool isInput, bool isMix) {
    ...
    std::for_each(port.profiles.begin(), port.profiles.end(),
        [&](const auto& profile) {
            dumpProfiles(profile, port.id);
        });
}

static std::vector<AudioProfile> populateProfiles(
    const std::variant<const xsd::MixPorts::MixPort, const xsd::DevicePorts::DevicePort>& audioPort) {
    ...
    auto getFormat =
        [](const xsd::Profile& profile) -> std::variant<PcmType, const std::string> {
    ...
    auto getChannels = [](const xsd::Profile& profile) {
        ...
        std::for_each(profile.getChannelLayouts().begin(),
            profile.getChannelLayouts().end(),
            [&](const auto& chLayout) {
            ...
            for (const auto& profile : port.getProfile()) {
                ...
                std::visit(overloaded {
                        [&](const std::string& formatStr) {
                            audioProfiles.push_back(createProfile(getName(profile),
                                formatStr, getChannels(profile),
                                profile.getSamplingRates(), getEncapsulationType(profile)));
                        },
                        [&](const PcmType& formatPcm) {
                            audioProfiles.push_back(createProfile(getName(profile),
                                formatPcm, getChannels(profile), profile.getSamplingRates(),
    ...
    std::visit(
        [&](const auto& port) {
            for (const auto& gain : port.getFirstGains()->getGain()) {
                auto g = createGain(getMode(gain),
                                    static_cast<int32_t>(getChannelLayout(gain)),
...
static void populateMixPorts(const xsd::Modules::Module& module,
    ...
    auto getFlags = [](const xsd::MixPorts::MixPort& mixPort, bool isInput) {
        ...
        std::for_each(flagVec.begin(), flagVec.end(),
            [&](const auto& flagStr) {
    ...
    for (const auto& mixPort : module.getFirstMixPorts()->getMixPort()) {
        ...
        auto port = createPort(moduleConfig->mNextPortId++, name, getFlags(mixPort, isInput), isInput,
        ...
        moduleConfig->mPorts.emplace_back(std::move(port));

        dumpPort(moduleConfig->mPorts.at(port.id), isInput, true);
    }
}
...
static void populateDevicePorts(const xsd::Modules::Module& module,
    ...
    auto getFlags = [](const xsd::DevicePorts::DevicePort& devPort) {
        if (devPort.hasDeviceType() && !devPort.hasConnection()) {
            if (devPort.getDeviceType() == xsd::AudioDeviceType::IN_MICROPHONE ||
    ...
    for (const auto& devicePort : module.getFirstDevicePorts()->getDevicePort()) {
        ...
        auto deviceExt = createDeviceExt(devType, getFlags(devicePort),
                                         getFormats(devicePort), getAddress(devicePort),
                                         getConnection(devicePort));
        auto port = createPort(moduleConfig->mNextPortId++, name, 0,
                               isInput, deviceExt);
        ...
        moduleConfig->mPorts.emplace_back(std::move(port));
        dumpPort(moduleConfig->mPorts.at(port.id), isInput, false);
    }
}
...
static void populateRoutes(const xsd::Modules::Module& module,
    ...
    for (const auto& route : module.getFirstRoutes()->getRoute()) {
        ...
        for (const auto& tag : srcTags) {
            auto srcItr = findPortByTagName(moduleConfig->mPorts.getCollection(), tag);
            if (srcItr != moduleConfig->mPorts.getCollection().end()) {
                sources.push_back(*srcItr);
            }
        }

        auto sink = findPortByTagName(moduleConfig->mPorts.getCollection(),
                                           route.getSink());
        LOG(VERBOSE) << __func__ << ": sink tag " << route.getSink();
        if (sink != moduleConfig->mPorts.getCollection().end()) {
            moduleConfig->mRoutes.push_back(createRoute(sources, *sink));
            dumpRoute(moduleConfig->mRoutes.back());
        }
    }
}
...
static void populateAttachedDevices(const xsd::Modules::Module& module,
    ...
    for (const auto& attachedDev : attachedDevices) {
        int32_t devPortId = findPortIdByTagName(moduleConfig->mPorts.getCollection(),
                                attachedDev);
        if (devPortId == -EINVAL) {
            LOG(WARNING) << __func__ << "Attached device port " << attachedDev
                    << " not found";
            continue;
        }
...
std::unique_ptr<ModuleConfig> getDefaultPrimaryConfiguration() {
    static const ModuleConfig configuration = []() {
        ...
        AudioPort telephonyTxOutDevice =
                createPort(c.mNextPortId++, "Telephony Tx", 0, false,
        ...
        AudioPort telephonyRxInDevice =
                createPort(c.mNextPortId++, "Telephony Rx", 0, true,
        ...
        AudioPort fmTunerInDevice = createPort(c.mNextPortId++, "FM Tuner", 0, true,
        ...
        AudioPort usbInDevice = createPort(c.mNextPortId++, "USB In", 0, true,
        ...
        AudioPort primaryOutMix = createPort(c.mNextPortId++, "primary output",
        ...
        AudioPort compressedOffloadOutMix =
                createPort(c.mNextPortId++, "compressed offload",
        ...
        AudioPort primaryInMix =
                createPort(c.mNextPortId++, "primary input", 0, true, createPortMixExt(2, 2));
        ...
        AudioPort telephonyTxOutMix =
                createPort(c.mNextPortId++, "telephony_tx", 0, false, createPortMixExt(1, 1));
        ...
        AudioPort telephonyRxInMix =
                createPort(c.mNextPortId++, "telephony_rx", 0, true, createPortMixExt(1, 1));
        ...
        AudioPort fmTunerInMix =
                createPort(c.mNextPortId++, "fm_tuner", 0, true, createPortMixExt(1, 1));
        fmTunerInMix.profiles.insert(fmTunerInMix.profiles.begin(),
                                     standardPcmAudioProfiles.begin(),
                                     standardPcmAudioProfiles.end());
        c.mPorts.emplace_back(std::move(fmTunerInMix));

        c.mRoutes.push_back(createRoute({primaryOutMix, compressedOffloadOutMix}, speakerOutDevice));
        c.mRoutes.push_back(
                createRoute({primaryOutMix, compressedOffloadOutMix}, usbOutDevice));
        c.mRoutes.push_back(createRoute({micInDevice, usbInDevice}, primaryInMix));
        c.mRoutes.push_back(createRoute({telephonyTxOutMix}, telephonyTxOutDevice));
        c.mRoutes.push_back(createRoute({telephonyRxInDevice}, telephonyRxInMix));
...
std::unique_ptr<ModuleConfig> getPrimaryConfiguration(){
    auto xsdConfig = xsd::read(getReadAbleConfigurationFile(kPrimaryConfigFileName).c_str());
...
std::unique_ptr<ModuleConfig> getRSubmixConfiguration(){
    auto xsdConfig = xsd::read(getReadAbleConfigurationFile(kRSubmixConfigFileName).c_str());
    if (!xsdConfig.has_value()) {
        LOG(WARNING) << __func__ << ": Rsubmix config retrieval failed, setting defaults";
        return std::move(getDefaultRSubmixConfiguration());
    }
    return std::move(getModuleConfig(xsdConfig.value()));
}

std::unique_ptr<ModuleConfig> getUsbConfiguration(){
    auto xsdConfig = xsd::read(getReadAbleConfigurationFile(kUSBConfigFileName).c_str());
    if (!xsdConfig.has_value()) {
        LOG(WARNING) << __func__ << ": usb config retrieval failed, setting defaults";
        return std::move(getDefaultUsbConfiguration());
    }
    return std::move(getModuleConfig(xsdConfig.value()));
}
...
std::unique_ptr<ModuleConfig> getDefaultRSubmixConfiguration() {
    static const ModuleConfig configuration = []() {
        ...
        AudioPort rsubmixOutDevice = createPort(c.mNextPortId++, "Remote Submix Out", 0, false,
                                                createDeviceExt(AudioDeviceType::OUT_SUBMIX, 0, {}));
        rsubmixOutDevice.profiles.push_back(
                createProfile("", PcmType::INT_24_BIT, {AudioChannelLayout::LAYOUT_STEREO}, {48000}));
        c.mPorts.emplace_back(std::move(rsubmixOutDevice));

        AudioPort rsubmixInDevice = createPort(c.mNextPortId++, "Remote Submix In", 0, true,
        ...
        AudioPort rsubmixOutMix =
                createPort(c.mNextPortId++, "r_submix output", 0, false, createPortMixExt(0, 0));
        rsubmixOutMix.profiles.push_back(
                createProfile("", PcmType::INT_24_BIT, {AudioChannelLayout::LAYOUT_STEREO}, {48000}));
        c.mPorts.emplace_back(std::move(rsubmixOutMix));

        AudioPort rsubmixInMix =
                createPort(c.mNextPortId++, "r_submix input", 0, true, createPortMixExt(0, 0));
...
std::unique_ptr<ModuleConfig> getDefaultUsbConfiguration() {
    static const ModuleConfig configuration = []() {
        ...
        c.mExternalDevicePortProfiles[usbOutHeadset.id] = standardPcmAudioProfiles;
        ...
        AudioPort usbHeadsetOutMix =
                createPort(c.mNextPortId++, "usb_headset output", 0, false, createPortMixExt(1, 1));
        usbHeadsetOutMix.profiles = standardPcmAudioProfiles;
        c.mPorts.emplace_back(std::move(usbHeadsetOutMix));

        AudioPort usbHeadsetInMix =
                createPort(c.mNextPortId++, "usb_headset input", 0, true, createPortMixExt(1, 1));
        usbHeadsetInMix.profiles = standardPcmAudioProfiles;
...
*/
int32_t ModuleConfig::dump(const int32_t fd) {
    std::ostringstream os;

    os << "--ModuleConfig dump start--" << std::endl;
    os << std::endl << "mPorts:" << std::endl;
    for (const auto& entry : this->mPorts) {
        os << entry.toString() << std::endl;
    }

    os << std::endl << "mExternalDevicePortProfiles:" << std::endl;
    for (const auto& [portId, profilesList] : this->mExternalDevicePortProfiles) {
    ...
    os << std::endl << "mActivePortConfigs:" << std::endl;
    for (const auto& entry : this->mActivePortConfigs) {
        os << entry.toString() << std::endl;
    }

    os << std::endl << "mRoutes:" << std::endl;
    for (const auto& entry : this->mRoutes) {
        os << entry.toString() << std::endl;
    }

    os << std::endl << "mPatches:" << std::endl;
    for (const auto& entry : this->mPatches) {
        os << entry.toString() << std::endl;
    }
    os << "--ModuleConfig dump end--" << std::endl;

 ...
 */
#define LOG_TAG "AHAL_AudioUsecaseExt"
...
int32_t CompressPlayback::getVendorParameters(
...
int32_t CompressPlayback::setVendorParameters(
...
std::unique_ptr<uint8_t[]> CompressPlayback::getPayloadCodecInfo() const {
...
int32_t CompressPlayback::getPresentationPosition(int64_t* positionInFrames) {
    ...
    constexpr uint64_t kMillion = 1 << 6;
    uint64_t dspFrames =
        static_cast<decltype(dspFrames)>(tstamp.session_time.value_msw);
    dspFrames = (dspFrames << 32 | tstamp.session_time.value_lsw) / kMillion;
    return 0;
}
...
int32_t CompressCapture::setVendorParameters(
    const std::vector<::aidl::android::hardware::audio::core::VendorParameter>&
        in_parameters,
    bool in_async) {
    auto getValueInt = [&](const std::string& searchKey)
...
int32_t CompressCapture::getVendorParameters(
    const std::vector<std::string>& in_ids,
    std::vector<::aidl::android::hardware::audio::core::VendorParameter>*
        _aidl_return) {
    return 0;
